...
export namespace C {
  ...
  const ACCEPTED_BODY_TYPES = 'application/sdp, application/dtmf-relay';
  const MAX_FORWARDS = 69;
  const SESSION_EXPIRES = 90;
  const MIN_SESSION_EXPIRES = 60;
  const CONNECTION_RECOVERY_MAX_INTERVAL = 30;
  const CONNECTION_RECOVERY_MIN_INTERVAL = 2;
}
...
export class UA {
  ...
  sendReInviteOnTransportConnect: boolean;
  sendRegisterOnTransportConnect: boolean;
  call(target: string | JsSIP.URI, options?: UserAgentCallOptions): any;
  get(parameter: string): string | undefined;
  createNewUATransport(config: JsSIP.UserAgentConfiguration): void;
  isConnected(): boolean;
  isRegistered(): boolean;
  register(): void;
  registrator(): JsSIP.Registrator;
  sendMessage(
    target: string | JsSIP.URI,
    body: string,
    options?: UserAgentSendMessageOptions,
  ): any;
  ...
  on(
    ...
    handler: EventHandler<UserAgentConnectingEvent>,
  ): void;
  on(
    eventName: 'connected',
    handler: EventHandler<UserAgentConnectedEvent>,
  ): void;
  on(
    eventName: 'disconnected',
    handler: EventHandler<UserAgentDisconnectedEvent>,
  ): void;
  on(
    eventName: 'registered',
    handler: EventHandler<UserAgentRegisteredEvent>,
  ): void;
  on(
    eventName: 'unregistered',
    handler: EventHandler<UserAgentUnregisteredEvent>,
  ): void;
  on(
    eventName: 'registrationFailed',
    handler: EventHandler<UserAgentRegistrationFailedEvent>,
  ): void;
  on(eventName: 'registrationExpiring', handler: () => void): void;
  on(
    eventName: 'newRTCSession',
    handler: EventHandler<UserAgentNewRtcSessionEvent>,
  ): void;
  on(
    eventName: 'newMessage',
    handler: EventHandler<UserAgentNewMessageEvent>,
  ): void;
...
export type SessionUserMediaFailedEvent = DOMException;
export type SessionCreateOfferFailedEvent = DOMException;
export type SessionCreateAnswerFailedEvent = DOMException;
export type SessionSetLocalDescriptionFailedEvent = DOMException;
export type SessionSetRemoteDescriptionFailedEvent = DOMException;
...
export interface IncomingMessage {
  ...
  parseHeader(name: string, idx?: number): object | undefined;
  toString(): string;
}
...
export class RTCSession {
  ...
  sendInfo(
    contentType: string,
    body?: string,
    options?: SessionSendInfoOptions,
  ): void;
  ...
  on(
    eventName: 'peerconnection',
    handler: EventHandler<RTCPeerConnection>,
  ): void;
  on(
    eventName: 'connecting',
    handler: EventHandler<SessionConnectingEvent>,
  ): void;
  ...
  on(
    eventName: 'confirmed',
    handler: EventHandler<SessionConfirmedEvent>,
  ): void;
  on(eventName: 'ended', handler: EventHandler<SessionEndedEvent>): void;
  on(eventName: 'noCall', handler: EventHandler<SessionEndedEvent>): void;
  ...
  on(
    ...
    handler: EventHandler<SessionIceCandidateEvent>,
  ): void;
  on(
    eventName: 'getusermediafailed',
    handler: EventHandler<SessionUserMediaFailedEvent>,
  ): void;
  on(
